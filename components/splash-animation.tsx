  ...
  useLayoutEffect(() => {
    ...
    class Bubble {
      ...
      draw(t: number) {
        ...
        const cx = this.ax + this.w / 2;
        const cy = this.ay + this.h / 2;
        const m = this.morph;
        ...
        // 对话结束：气泡原地淡出（不再变成任何东西飘走）
        const fadeA = 1 - clamp((m - 0.15) / 0.55, 0, 1);
        if (fadeA <= 0.01) {
          ctx.restore();
          return;
        }
        ...
        if (edgeBlur > 0.06 && useSoftEdgeFallback) {
          ...
          for (const pass of passes) {
            ctx.globalAlpha = fadeA * pass.alpha;
        ...
        ctx.shadowColor = "transparent";
        if (m > 0.3) {
          ctx.globalAlpha = fadeA * (isRight ? 0.5 : 0.55);
